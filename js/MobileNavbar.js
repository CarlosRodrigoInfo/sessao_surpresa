...
let currentPage = Math.floor(Math.random() * 100) + 1; // Gera um número aleatório entre 1 e 10 para a página
...
function getRandomMedia() {
  ...
  const rating = '6.0'; // Avaliação fixa 

  // Define o tipo de mídia com base na seleção
  let searchMediaType;
  if (mediaType === 'filme') {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  form.addEventListener('submit', function (event) {
    ...
    currentPage = Math.floor(Math.random() * 100) + 1; // Gera um número aleatório entre 1 e 1000 para a página ao enviar o formulário
    getRandomMedia();
  });
  getRandomMedia();
});
